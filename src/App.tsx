...
function App() {
	...
	const [dialogLabelByRow, setDialogLabelByRow] = useState<
		Record<number, string>
	>({
		1: '',
		2: '',
	...
	const handleDialogSubmit = () => {
		if (selectedRadio !== '' && selectedRow !== null) {
			const selectedOption = options.find(
				(option) => String(option.value) === selectedRadio,
			);
					...
					<Grid>
						{selectedRow ? (
							<Box sx={{ p: 2, border: '1px solid #ccc', borderRadius: 2 }}>
								<h2>
									Form {tableRows.find((r) => r.id === selectedRow)?.name}
								</h2>
								...
								<Autocomplete
									...
									value={
										selectedRow !== null ? initialFormValues[selectedRow] : ''
									}
